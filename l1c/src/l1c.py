...
class l1c(initL1c):
    ...
    def processModule(self):
        ...
        for band in self.globalConfig.bands:
            ...
            #plotL1cToa(self, lat_l1c, lon_l1c, toa_l1c, outdir, band)
            self.plotL1cToa(lat_l1c,lon_l1c,toa_l1c,self.outdir,band)
    ...
    def l1cProjtoa(self, lat, lon, toa, band):
        ...
        for ialt in range(toa.shape[1]):
            for iact in range(toa.shape[0]):
                my_mgrs = str(m.toMGRS(lat[iact,ialt],lon[iact,ialt], inDegrees=True, MGRSPrecision= self.l1cConfig.mgrs_tile_precision))
    ...
    def plotL1cToa(self, lat_l1c, lon_l1c, toa_l1c, outdir, band):
